...
class SettingsAdmin(admin.ModelAdmin):
    list_display = ('site_name', 'total_orders', 'total_transactions',
                    'total_users','todays_fund_recieved','todays_update','todays_new_user','tickets')
    ...
    def todays_update(self,request):
        ...
        orders =  ordersObj.filter(Q(status__in=["Completed","Processing","In Progress"]),Q(order_placed__range=[startdate, enddate]))
    ...
    def tickets(self,request):
        ...
        return format_html(f"<a href='/admin/services/ticketsmodel/' >{ticket} Unanswered</a>")